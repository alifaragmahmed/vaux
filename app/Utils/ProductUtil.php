...
class ProductUtil extends Util
{
        ...
        foreach ($input_variations as $key => $value) {
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_value_id2 = !empty($v['variation_value_id2']) ? $v['variation_value_id2'] : null;
                    $variation_value_name = !empty($v['value']) ? $v['value'] : null;

                    $variation_value_name_input = !empty($v['value']) ? $v['value'] : null;
                    $variation_template_id2 = !empty($v['variation_template_id2']) ? $v['variation_template_id2'] : null;  
                    ...
                    $variation_data[] = [
                      'name' =>  $variation_value_name_input,// old value => $variation_value_name,
                      'variation_value_id' => $variation_value_id,
                      'variation_value_id2' => $variation_value_id2,
                      'variation_template_id2' => $variation_template_id2,
                      'product_id' => $product->id,
                      'business_id' => $business_id,
                      'sub_sku' => $sub_sku,
        ...
        foreach ($input_variations_edit as $key => $value) {
            ...
            if (!empty($value['variations_edit'])) {
                foreach ($value['variations_edit'] as $k => $v) {
                    $data = [
                        'name' => $v['value'],
                        'variation_value_id2' => $v['variation_value_id2'],
                        'variation_template_id2' => $v['variation_template_id2'],
                        'default_purchase_price' => $this->num_uf($v['default_purchase_price']),
                        'dpp_inc_tax' => $this->num_uf($v['dpp_inc_tax']),
                        'profit_percent' => $this->num_uf($v['profit_percent']),
                        'default_sell_price' => $this->num_uf($v['default_sell_price']),
     ...
     */
    public function generateProductSku($string, $business_id = null) 
    {
