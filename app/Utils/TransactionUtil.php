...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'shipping_company_id' => isset($input['shipping_company_id']) ? $input['shipping_company_id'] : null,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //echo "<pre>";print_r($customer->toArray());exit;
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_draftno'] = $customer->custom_field3;
            $output['customer_pono'] = $customer->custom_field2;
            $output['customer_billto'] = $customer->custom_field4;
    ...
    */
    ...
    public function getTransactionTotals(
        ...
        if (in_array('expense', $transaction_types)) {
            ...
            //
            $output['expense_rows'] = Transaction::where('business_id', $business_id)
                ->select(
                    DB::raw('(select name from expense_categories where id = expense_category_id) as expense_name'),
                    DB::raw("SUM(IF(transactions.type='expense', final_total, 0)) as total_expense"), 
                )
                ->where('type', 'expense')
    ...
    */
    public function getListSells($business_id)
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('shipping_companies as com', 'transactions.shipping_company_id', '=', 'com.id')
                ...
                ->select(
                    'transactions.id',
                    'transactions.shipping_company_id',
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null)
    {
        ...
        $data['expense_rows'] = !empty($transaction_totals['expense_rows']) ? $transaction_totals['expense_rows'] : [];
        //Discounts
        ...
        // new edits
        $data['cogs'] = ($data['opening_stock'] + $data['total_purchase']) - $data['closing_stock'];
        $data['gross_profit'] = $data['total_sell'] - $data['cogs'];
        $data['other_income'] = $data['total_sell_shipping_charge'] + $data['total_recovered'] + $data['total_purchase_return'] + $data['total_purchase_discount'] + $data['total_sell_round_off'];

        // net profit ((Gross Profit + other income ) - G&A)
        $data['net_profit'] = ($data['gross_profit'] + $data['other_income']) - $data['total_expense'];
        $data['total_revenue'] = $data['total_sell'] + $data['other_income'] + $data['total_sell_discount'];
        $data['total_expense'] = $data['total_expense'] + $data['total_transfer_shipping_charges'];

        //total_cost_of_sales = (opening_stock + total_purchase_shipping_charge + total_adjustment) - (total_purchase_return + total_purchase_discount)
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        $sell_return_data = [
            ...
            'tax_id' => $input['tax_id'],
            'tax_amount' => isset($invoice_total['tax'])? $invoice_total['tax'] : 0,
            'total_before_tax' => isset($invoice_total['total_before_tax'])? $invoice_total['total_before_tax'] : 0,
            'final_total' => isset($invoice_total['final_total'])? $invoice_total['final_total'] : 0,
            'reason_notes' => isset($input['reason_notes'])? $input['reason_notes'] : '',
        ];
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['reason_notes'] = isset($input['reason_notes'])? $input['reason_notes'] : '';
        ...
        $returns = [];
        $product_lines = isset($input['products'])? $input['products'] : [];
