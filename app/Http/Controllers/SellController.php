...
class SellController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            if (!empty(request()->input('shipping_company'))) {
                $sells->where('transactions.shipping_company_id', request()->input('shipping_company'));
            }
            ...
            $datatable = Datatables::of($sells->latest('transactions.created_at'))
                ->addColumn(
                    'action',
                    function ($row) use ($only_shipments, $is_admin) {
                        ...
                        if (!$only_shipments) {
                            ...
                            if($row->shipping_company_id != null && $row->payment_status !== "paid"){
                                $html .= '<li><a href="#" data-href="' . action('ShippingSettlementController@update', ["company" => $row->shipping_company_id,"id" => $row->id]) . '" class="btn-modal settle_shipping_company_button"><i class="fa fa-envelope" aria-hidden="true"></i>' . __("shipping.settle") . '</a></li>';
                            }
                        } else {
            ...
            $rawColumns = ['final_total', 'action', 'mass_delete', 'total_paid', 'total_remaining', 'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 'total_before_tax', 'shipping_status', 'types_of_service_name', 'payment_methods', 'return_due', 'conatct_name', 'settle'];
        ...
        $companies = ShippingCompany::where('business_id', $business_id)->pluck('name', 'id');

        return view('sell.index')
        ->with(compact('business_locations', 'companies', 'customers', 'is_woocommerce', 'sales_representative', 'is_cmsn_agent_enabled', 'commission_agents', 'service_staffs', 'is_tables_enabled', 'is_service_staff_enabled', 'is_types_service_enabled', 'shipping_statuses'));
    }
    ...
    public function create()
    {
        ...
        $companies = ShippingCompany::forDropdown($business_id);
