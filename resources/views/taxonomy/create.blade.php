    ...
    <div class="modal-body">
      ...
      @if(!empty($parent_categories) && $enable_sub_category)
        <div class="form-group">
            <div class="checkbox">
              <label>
                 {!! Form::checkbox('add_as_sub_cat', 1, false,[ 'class' => 'category-check toggler icheck', 'data-toggle_id' => 'parent_cat_div' ]); !!} @lang( 'lang_v1.add_as_sub_txonomy' )
              </label>
            </div>
        </div>
...
<script>
  ...
  $('.category-check').iCheck('update', function(){
    if ($('.category-check').iCheck('check')) {
