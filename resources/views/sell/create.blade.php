...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid w3-padding'])
				...
				@if(in_array('subscription', $enabled_modules))
					<div class="col-sm-4">
						<div class="checkbox w3-round w3-border w3-border-light-gray" style="margin: 0px!important;padding-left: 5px" >
							<label>
				...
				<div class="col-sm-4 @if(!empty($commission_agent)) c @else c @endif">
					<div class="form-group">
						...
						<div class="input-group"> 
							...
							<span class="input-group-btn">
								<button type="button" class="w3-margin-left btn btn-default add_select_btn w3-round btn-modal new-theme add_new_customer" data-name="">
					...
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-btn">
								...
								style="margin-right: 5px" data-toggle="modal" data-target="#configure_search_modal" title="{{__('lang_v1.configure_product_search')}}">
									<i class="fa fa-barcode w3-text-white"></i>
								</button>
							</div>
							...
							<span class="input-group-btn">
								...
								class="btn btn-default add_select_btn w3-round btn-modal new-theme pos_add_quick_product" 
								data-href="{{action('ProductController@quickAdd')}}" 
								data-container=".quick_add_product_modal">
								<i class="fa fa-plus-circle text-primary fa-lg w3-text-white"></i>
							</button>
							</span>
						</div>
					</div>
			...
			@endphp
			    ...
			    <div class="hidden col-md-12 well well-sm bg-light-gray @if(session('business.enable_rp') != 1) hide @endif">
			...
			<div class="col-md-4">
				<div class="form-group">
					{!! Form::label('shipping_company_id', __('shipping.shipping_company') . ':' ) !!}
					{!! Form::select('shipping_company_id',$companies, null, ['class' => 'form-control','placeholder' => __('messages.please_select'), 'id' => 'shipping_company']); !!}
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					{!! Form::label('shipping_company_id', __('shipping.areas') . ':' ) !!}
					{!! Form::select('area_id',$areas, null, ['class' => 'form-control','placeholder' => __('messages.please_select'), 'id' => 'shipping_area', 'disabled']); !!}
					<p class="help-block">Please select a company first.</p>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					...
					{!!Form::text('shipping_charges',@num_format(0.00),['class'=>'form-control input_number','placeholder'=> __('sale.shipping_charges'), 'id' => 'shipping_charges']);!!}
					</div>
				</div>
			</div>
			<div class="clearfix"></div>
	...
	@if(empty($status) || !in_array($status, ['quotation', 'draft']))
		@can('sell.payments')
			@component('components.widget', ['class' => 'box-solid w3-padding', 'id' => "payment_rows_div", 'title' => ''])
			<div class="text-center w3-large w3-padding">
				{{ __('purchase.add_payment') }}
			</div>
			<br>
			<div class="payment_row">
				<div class="row">
					<div class="col-md-12 mb-12 text-center w3-large">
				...
				<div class="row">
					<div class="col-sm-12 text-center w3-large">
						<div class=""><strong>@lang('lang_v1.balance'):</strong> <span class="balance_due">0.00</span></div>
					</div>
				</div>
			</div>
			@endcomponent
	...
	<center class="">
		...
		<div class="">
			<button type="button" id="submit-sell" class="add_btn">@lang('messages.save')</button>
			<button type="button" id="save-and-print" style="width: 220px" class="add_btn">@lang('lang_v1.save_and_print')</button>
		</div>
	</center>
    ...
    <script type="text/javascript">
    	$(document).ready( function() {
			...
			$('#shipping_company').change(function(){
    			if ($(this).val() != '') {
					$("#shipping_area").prop('disabled', false);
    			} else {
			...
			$('#shipping_area').change(function(){
				...
				let company_id = $("#shipping_company").val();
				let url = '{{ route("getDelivery.fees", ["company" => ":company_id" , "area" => ":area_id"]) }}';
                    url = url.replace(':area_id', area_id).replace(':company_id', company_id);
				...
				$.ajax({
					...
					success: function(result) {
						if (result.success == true) {
							$("#shipping_charges").val(result.amount);
