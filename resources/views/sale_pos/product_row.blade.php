<tr class="product_row product-row-number-{{$row_count}}" data-row_index="{{$row_count}}">
	<td>
		...
		<div title="@lang('lang_v1.pos_edit_product_price_help')" class=" w3-text-dark-gray" > 
			<img 
			src="
			@if(count($p->media) > 0)
			...
			<b style="padding: 5px" >{{ $this_product->name }}
				<span 
	...
	<td style="padding: 12px">
		<p class="font-weight-bolder">
			<b>$<b data-price="{{ $product->default_sell_price }}" class="row-total" >{{ $product->default_sell_price * $quantity }}</b></b>
		</p>
	</td>
