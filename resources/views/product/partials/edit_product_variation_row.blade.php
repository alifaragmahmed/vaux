...
$variationIds = App\VariationTemplate::where('business_id', $business_id)->select('id')->pluck('id')->toArray();
$variationValues = App\VariationValueTemplate::whereIn('variation_template_id', $variationIds)->get();
@endphp
        ...
        <table class="table table-condensed table-bordered blue-header variation_value_table">
            ...
            @forelse ($product_variation->variations as $variation)
                ...
                <tr>
                    ...
                    <td>
                        {!! Form::text($array_name . '[' . $row_index .'][' . $variation_array_name . '][' . $variation_row_index . '][value]', $variation->name, ['data-value'=> $variation_name, 'class' => 'form-control input-sm variation_value_name variation_value_name value-row-'.$value_index, 'required', 'readonly']); !!}
                    ...
                    <td class="w3-display-container" >
                        ...
                        <div class="@if($value_index == 0) input-group @endif">
                            <select  
                            class="@if($value_index == 0) input-group-addon @endif var-temp-select var-temp-select-{{ $value_index }}"
                            onchange="$('.var-temp').hide();$('.var-temp-'+this.value).show();" 
                            name="{{ $array_name . '[' . $row_index .'][' . $variation_array_name . '][' . $variation_row_index . '][variation_template_id2]' }}" 
                            id="">
                                @foreach ($variations as $item)
                                    <option value="{{ $item->id }}" {{ $variation->variation_template_id2 == $item->id? 'selected' : '' }} >{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @if($value_index == 0)
                            <span class="input-group-addon">
                                <button 
                                type="button" 
                                onclick="$('.var-temp-select').val($('.var-temp-select-{{ $value_index }}').val());"
                                class="fas fa-check-double" ></button>
                            </span>
                        ...
                        <script>
                            $('.var-temp-select-{{ $value_index }}').change();
                        </script>
                    </td>
                    <td>
                        <div class="@if($value_index == 0) input-group @endif">
                            <select 
                            class="@if($value_index == 0) input-group-addon @endif var-temp-val-select var-temp-val-select-{{ $value_index }}"
                            onchange="$('.value-row-{{ $value_index }}').val($('.value-row-{{ $value_index }}').attr('data-value') + '-' + $('.var-temp-val-'+this.value)[0].innerText)" 
                            name="{{ $array_name . '[' . $row_index .'][' . $variation_array_name . '][' . $variation_row_index . '][variation_value_id2]' }}" 
                            id="">
                                <option value=""></option>
                                @foreach ($variationValues as $item)
                                    <option value="{{ $item->id }}" 
                                        {{ $variation->variation_value_id2 == $item->id? 'selected' : '' }}
                                        class="var-temp-val-{{ $item->id }} var-temp var-temp-{{ $item->variation_template_id }}" style="display: none" >{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @if($value_index == 0)
                            <span class="input-group-addon">
                                <button 
                                type="button" 
                                onclick="$('.var-temp-val-select').val($('.var-temp-val-select-{{ $value_index }}').val());$('.var-temp-val-select').change()"
                                class="fas fa-check-double" ></button>
                            </span>
