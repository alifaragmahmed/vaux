...
<tr>
    <td>
        {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][sub_sku]', null, ['class' => 'form-control input-sm name-row-'.$value_index]); !!}

        {!! Form::hidden($name . '[' . $variation_index . '][variations][' . $value_index . '][variation_value_id]', $variation_value_id) !!}
    </td>
    <td>
        {!! Form::text($name . '[' . $variation_index . '][variations][' . $value_index . '][value]', $variation_name, ['data-value'=> $variation_name, 'class' => 'form-control input-sm variation_value_name value-row-'.$value_index, 'required', $readonly]); !!}
    </td>
    <td class="w3-display-container" >
        ...
        <div class="@if($value_index == 0) input-group @endif">
            ...
            name="{{ $name . '[' . $variation_index . '][variations][' . $value_index . '][variation_template_id2]' }}" id="">
    ...
    <td>
        <div class="@if($value_index == 0) input-group @endif">
            ...
            name="{{ $name . '[' . $variation_index . '][variations][' . $value_index . '][variation_value_id2]' }}" id="">
                <option value=""></option>
                @foreach ($variationValues as $item)
                    <option value="{{ $item->id }}" class="var-temp-val-{{ $item->id }} var-temp var-temp-{{ $item->variation_template_id }}" style="display: none" >{{ $item->name }}</option>
                @endforeach
            </select>
            @if($value_index == 0)
            <span class="input-group-addon">
