$(document).ready(function() {
    ...
    //Finalize with cod method
    $('#pos-cod-finalize, .pos-cod-finalize-action').click(function() {
        ...
        var data = pos_form_obj.serializeArray();
        data.push({ name: 'payment[0][amount]', value: '0' });
        data.push({ name: 'payment[0][method]', value: 'cash' });
        data.push({ name: 'payment[0][card_type]', value: 'credit' });
    ...
    $('#pos-save-card').on("click", function() {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == 1) {
                            ...
                            // check if result.whatsapp_link has value
                            if (result.whatsapp_link) {
                                window.open(result.whatsapp_link, '_blank').focus();
                            }
                            toastr.success(result.msg);
    ...
    //Update discount
    $('#posEditDiscountModalUpdate').click(function() {
    ...
    //Shipping
    $('#posShippingModalUpdate').click(function() {
        //Close modal
    ...
    $('#submit-sell, #save-and-print').click(function(e) {
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
            return false;
    ...
    $('#weighing_scale_modal').on('shown.bs.modal', function(e) {
        ...
        onScan.attachTo(document, {
            ...
            onScan: function(sCode, iQty) {
                ...
                $('#weighing_scale_submit').trigger('click');
            },
    ...
    $('#weighing_scale_submit').click(function() {
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    var selector = ".product-item" + variation_id;

    var replace = $(selector).attr('replace');
