...
class WoocommerceUtil extends Util
{
    ...
    public function syncProducts($business_id, $user_id, $sync_type, $limit = 100, $page = 0)
    {
        ...
        $query = Product::where('business_id', $business_id)
                        ...
                        ->with(['variations', 'category', 'sub_category',
                            'variations.variation_template_value_2','variations.variation_template_2',
            ...
            if ($product->type == 'variable') {
                ...
                foreach ($product->variations as $variation) {
                    if (!empty($variation->product_variation->variation_template->woocommerce_attr_id)) {
                        ...
                        if($variation->variation_template_2 && $variation->variation_template_value_2){
                            $woocommerce_attr_id2 = $variation->variation_template_2->woocommerce_attr_id;
                            $variation_attr_data[$woocommerce_attr_id2][] = $variation->variation_template_value_2->name;
                        }
                    }
                ...
                foreach ($variation_attr_data as $key => $value) {
                    // TODO: Add anthor attribute here
                    $array['attributes'][] = [
                        'id' => $key,
                        'variation' => true,
                        'visible'   => true,
    ...
    public function syncProductVariations($business_id, $sync_type = 'all', $new_woocommerce_product_ids = [])
    {
        ...
        $query = Product::where('business_id', $business_id)
                        ...
                        ->where('woocommerce_disable_sync', 0)
                        ->with(['variations',
                            'variations.variation_template_2',
                            'variations.variation_location_details',
                            'variations.product_variation',
                            'variations.product_variation.variation_template']);
        ...
        foreach ($variable_products as $product) {
            ...
            foreach ($variations as $variation) {
                ...
                // TODO: Add variations attributes here
                if (!empty($variation->product_variation->variation_template->woocommerce_attr_id)) {
                    ...
                    if($variation->variation_template_2 && $variation->variation_template_value_2){
                        $variation_arr['attributes'][] = [
                            'id' => $variation->variation_template_2->woocommerce_attr_id,
                            'option' => $variation->variation_template_value_2->name
                        ];
                    }
                }
     ...
     * @param [type] $business_data
     * @return \App\Product|null
     */
    public function createProduct($business_id, $user_id, $requestData, $business_data)
    {
        // dd($requestData->variations);
        $productCreated = null;
        /**
         * check type of product
         * @example product_type single => simple || variable => { variable || variation }
         */
        // if product type is simple
        if ($requestData->type === 'simple') {
            // Check if product is created before
            $product = Product::where('business_id', $business_id)->where('woocommerce_product_id', $requestData->id)->exists();
            if ($product) {
                return null;
            }
            // create new product
            # code...
            $productCreated = $this->AddNewProduct($requestData, $business_id, $user_id);
        } else if ($requestData->type === 'variation') {

            // check if variations exists

            $variations = Variation::where('business_id', $business_id)->where('woocommerce_variation_id', $requestData->id)->exists();
            if ($variations) {
                return null;
            }
            // check if attribute is exist or create new one
            $variationTemplate = $this->addProductVariantion($requestData, $business_id, $user_id);

            // create new variation
            
        } else if ($requestData->type === 'variable') {
            // Check if product is created before
            $product = Product::where('business_id', $business_id)->where('woocommerce_product_id', $requestData->id)->exists();
            if ($product) {
                return null;
            ...
            $productCreated = $this->AddNewProduct($requestData, $business_id, $user_id);
            Variation::where('business_id', $business_id)->whereIn('woocommerce_variation_id', $requestData->variations)->update([
                'product_id' => $productCreated->id,
            ]);
            ProductVariation::whereHas('variations', function ($q) use ($business_id, $requestData) {
                $q->where('business_id', $business_id)->whereIn('woocommerce_variation_id', $requestData->variations);
            })->update([
                'product_id' => $productCreated->id,
            ]);
        }
        // else if product type is variable
        //      1. create attribute 
     ...
     * @return void
     */
    public function addProductVariantion($requestData, $business_id, $user_id)
    {
        // dd($requestData);
        $attributesArr = [];

        foreach ($requestData->attributes as $key => $attribute) {
            $attributeCreated = VariationTemplate::firstOrCreate([
                'woocommerce_attr_id' => $attribute->id,
                'business_id' => $business_id
            ],[
                'name' => $attribute->name,
            ]);
            $optionCreated = VariationValueTemplate::firstOrCreate([
                'variation_template_id' => $attributeCreated->id,
                'name' => $attribute->option,
            ]);
            $attributesArr[$key]['variation_template_id'] = $attributeCreated->id;
            $attributesArr[$key]['variation_template_name'] = $attributeCreated->name;
            $attributesArr[$key]['variation_value_id'] = $optionCreated->id;
            $attributesArr[$key]['variation_value_name'] = $optionCreated->name;
        }

        $productVariation = ProductVariation::create([
            // 'product_id' => $product->id,
            'variation_template_id' => $attributesArr[0]['variation_template_id'],
            'name' =>  $attributesArr[0]['variation_template_name'],
            'is_dummy' => true,
        ]);
        $variantion = Variation::create([
            // 'product_id' => $product->id,
            'name' =>  $attributesArr[0]['variation_value_name'],
            'business_id' => $business_id,
            'sub_sku' => $requestData->sku,
            'product_variation_id' => $productVariation->id,
            'woocommerce_variation_id' => $requestData->id,
            'variation_value_id' =>  $attributesArr[0]['variation_value_id'],
            'default_sell_price' => $requestData->regular_price,
            'sell_price_inc_tax' => $requestData->regular_price,
            'variation_template_id2' => $attributesArr[1]['variation_template_id'],
            'variation_value_id2' => $attributesArr[1]['variation_value_id'],
        ]);
     ...
     * @return object
     */
    private function AddNewProduct($requestData, $business_id, $user_id)
    {
        # STEP 1
        // check if category exists then create category
        $category = Category::where('business_id', $business_id)->where('woocommerce_cat_id', $requestData->categories[0]->id)->first();
        if(!$category){
            $category = Category::create([
                        'name' => $requestData->categories[0]->name,
                        'woocommerce_cat_id' => $requestData->categories[0]->id,
                        'slug' => $requestData->categories[0]->slug,
                        'business_id' => $business_id,
        ...
        $product = Product::create([
            ...
            'type' => $requestData->type == 'simple' ? 'single' : 'variable',
            'business_id' => $business_id,
            'category_id' => $category->id,
            'enable_stock' => $requestData->manage_stock,
            'barcode_type' => 'C128',
            'sku' => $requestData->sku ?? '',
            'weight' => $requestData->weight,
            // 'image' => $requestData->images[0]->src, // TODO: save image in our server and put our image url
            'woocommerce_media_id' => $requestData->images[0]->id ?? '',
            'product_description' => $requestData->description,
            'created_by' => $user_id,
            'woocommerce_product_id' => $requestData->id,
            'unit_id' => $unit->id, //required to show in list
        ]);

        // Generate product sku
        if (empty(trim($requestData->sku))) {
            $sku = $this->productUtil->generateProductSku($product->id, $business_id);
            $product->sku = $sku;
            $product->save();
        }

        # STEP 3 required to show in list
        if($requestData->type == 'simple'){
            // create product variation with variation_template_id = null
            ...
            # STEP 4 required to show in list
            // create variation with product_variation_id get it from last query
    ...
    public function woocommerceOrderStatusToPosSellStatus($status, $business_id)
    {
        ...
        $sale_status = empty($sale_status) && array_key_exists($status, $status_array) ? $status_array[$status] : $sale_status;
