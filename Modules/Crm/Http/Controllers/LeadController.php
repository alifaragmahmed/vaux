    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            if ($lead_view == 'list_view') {
                // return (Datatables::of($leads)->removeColumn('id')->make(true));
                return (Datatables::of($leads)
                    ...
                    ->addColumn('last_followed_up_on',function($row){
                        $html= '';
                        if(!empty($row->last_followed_up_on)){
                            $html.=$this->commonUtil->format_date($row->last_followed_up_on,true);;
                            $html.= '<br><a href="'.action('\Modules\Crm\Http\Controllers\ScheduleController@show', ['follow_up' => $row->last_followed_up_on]).'" target="_blank" title="'.__("crm::lang.view_follow_up").'" data-toggle="tooltip">
                                <i class="fas fa-external-link-alt"></i>
                            </a> <br>';
                        }
                        $html.='<a href="#" data-href="'.action('\Modules\Crm\Http\Controllers\ScheduleController@create', ["schedule_for"=>"lead", "contact_id"=>$row->id]).'" class="btn-modal btn btn-xs btn-primary" data-container=".schedule">
                    ...
                    ->orderColumn('last_followed_up_on', function ($query, $order) {
                        $query->orderBy('last_followed_up_on', $order);
                    })
                    ...
                    ->rawColumns(['action', 'crm_source', 'crm_life_stage', 'leadUsers', 'last_follow_up', 'upcoming_follow_up','last_followed_up_on', 'created_at', 'name'])
                    ->make(true));
